...
"""
print("🔥 USING CUSTOM TRAINER.PY")
import sys
sys.path.insert(0, r"C:\Users\user\Downloads\firdge_Distiller\yolo-distiller-main")
    ...
    def __init__(self,
        ...
        self.generation = nn.ModuleList([
            nn.Sequential(
                ...
                nn.Conv2d(t_chan, t_chan, kernel_size=3, padding=1)
            ).to(device) for t_chan in teacher_channels
        ])
...
class BaseTrainer:
    ...
    def _do_train(self, world_size=1):
        ...
        # Find rotten spinach class index once
        self.rotten_id = None
        if isinstance(self.data["names"], dict):
            for class_id, class_name in self.data["names"].items():
                if class_name == "rotten spinach":
                    self.rotten_id = class_id
                    break
        else:
            try:
                self.rotten_id = self.data["names"].index("rotten spinach")
        ...
        while True:
            ...
            self.tloss = None
            self.weighted_kd_loss = torch.tensor(0.0, device=self.device)
            ...
            for i, batch in pbar:
                ...
                with autocast(self.amp):
                    ...
                    if self.rotten_id is not None and len(cls):
                        weight = torch.ones_like(cls, dtype=torch.float32, device=cls.device)
                        weight[cls == self.rotten_id] = 2.5
                        self.loss = self.loss * weight.mean()
                ...
                self.weighted_kd_loss = torch.tensor(0.0, device=self.device)
                self.d_loss = torch.tensor(0.0, device=self.device)
                self.d_loss_item = 0.0
                self.distill_weight = 1.0

                if getattr(self, "teacher", None) is not None:

                    denom = max(1, len(self.train_loader))
                    self.distill_weight = ((1 - math.cos(i * math.pi / denom)) / 2) * (0.1 - 1) + 1
                    ...
                    loss = distillation_loss.get_loss()
                    ...
                    self.d_loss = loss
                    self.d_loss_item = loss.item()

                    self.weighted_kd_loss = loss * self.distill_weight

                    self.loss = self.loss + self.weighted_kd_loss
            ...
            if RANK in {-1, 0}:
                if getattr(self, "teacher", None) is not None:
                    LOGGER.info(
                        f"Epoch {epoch + 1} KD raw: {self.d_loss_item:.6f} | "
                        f"KD weight: {self.distill_weight:.6f} | KD active: True"
                    )
